  ...
  try {
    ...
    if (body.message && body.message.text) {
      ...
      if (text.startsWith('/status')) {
        ...
        const dashboardUrl = `${dashboardBaseUrl}?hideoverlay=true&t=${Date.now()}`;
        const microlinkUrl = `https://api.microlink.io?url=${encodeURIComponent(dashboardUrl)}&screenshot=true&meta=false&embed=screenshot.url&waitUntil=networkidle0&delay=8000&adblock=false&force=true`;
