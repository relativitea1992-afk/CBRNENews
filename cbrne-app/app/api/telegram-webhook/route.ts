    ...
    if (text && chatId) {
      ...
      const generateResourceReport = async (days: number) => {
        ...
        try {
          ...
          msg += `  └ Total Words Scanned: ${totalWordCount.toLocaleString()}\n`;
        ...
        after(async () => {
          try {
            ...
            const promises = [
              ...
              ping('Gemini APIs', 'Gemini (3.8-flash)', 'https://generativelanguage.googleapis.com', async () => {
                const key = process.env.GEMINI_API_KEY;
                if (!key) throw new Error('No Key');
                const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.8-flash?key=${key}`);
              ...
              ping('Gemini APIs', 'Gemini (3-flash-preview)', 'https://generativelanguage.googleapis.com', async () => {
                const key = process.env.GEMINI_API_KEY;
                if (!key) throw new Error('No Key');
